    ...
    public class WikiParser
    {
        ...
        public int ParseItemWikiPage(HtmlNode itemBoxNode)
        {
            ...
            if (dlNode != null)
            {
                ...
                try
                {
                    ...
                    if (idList.Contains(","))
                    {
                        return int.Parse(idList.Split(",", StringSplitOptions.RemoveEmptyEntries)[0]);
                    }
                    return int.Parse(idList);
                }
                catch (Exception ex)
                {
                    throw;
        ...
        public CollectionAchievementTable? ParseCollectionAchievementPage(HtmlNode collectionAchievementPageNode)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < tableHeaderCells.Length; i++)
                {
            ...
            for (int i = 0; i < rows.Length; i++)
            {
                ...
                {
                    var entry = ParseCollectionAchievementTableEntry(tableData[j]);
        ...
        private CollectionAchievementTable.CollectionAchievementTableEntry ParseCollectionAchievementTableEntry(HtmlNode entry)
        {
            ...
            foreach (var item in entry.ChildNodes)
            {
                ...
                {
                    if (int.TryParse(SanitizesDisplayName(item.InnerText), out var number))
                    {
                        return new CollectionAchievementTable.CollectionAchievementTableNumberEntry()
                        {
                            Number = number,
                        };
                    }

                    return new CollectionAchievementTable.CollectionAchievementTableStringEntry()
                    {
                ...
                else if (item.Name == "a")
                {
                    ...
                    else
                    {
                        ...
                        if (imageNode != null)
                        {
                            currentEntry = new CollectionAchievementTable.CollectionAchievementTableMapEntry()
                            {
                        ...
                        {
                            currentEntry = new CollectionAchievementTable.CollectionAchievementTableLinkEntry
                            {
                ...
                else if (item.Name == "i")
                {
                    ...
                    stringEntry.Text = item.InnerText;
